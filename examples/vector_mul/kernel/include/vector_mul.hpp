...
/*
 * This is the most basic single tile version of vector multiplication
 * that multiplies two vectors A and B and stores the result in C.
 *
 * The code assumes a sinlge 1x1 grid of a single 1x1 tile group
 * Do NOT use this version with larger tile groups 
 */
template <typename TA, typename TB, typename TC>
int __attribute__ ((noinline)) kernel_vector_mul_single_tile(TA *A, TB *B, TC *C,
                      uint32_t WIDTH) {
        // A single tile performs the entire vector multiplication
...
/*
 * This is the single 1 dimensional tile group version of vector multiplication
 * that multiplies two vectors A and B and stores the result in C.
 *
 * The code assumes a sinlge 1x1 grid of 1-dimensional tile group
 */
template <typename TA, typename TB, typename TC>
int __attribute__ ((noinline)) kernel_vector_mul_single_1D_tile_group(TA *A, TB *B, TC *C,
                      uint32_t WIDTH) {

        // Vector is divided among tiles in the tile group
        // As tile group is one dimensional, each tile performs
        // (WIDTH / bsg_tiles_X) multiplies
...
/*
 * This is the single 2 dimensional tile group version of vector multiplication
 * that multiplies two vectors A and B and stores the result in C.
 *
 * The code assumes a sinlge 1x1 grid of tile group
 */
template <typename TA, typename TB, typename TC>
int __attribute__ ((noinline)) kernel_vector_mul_single_2D_tile_group(TA *A, TB *B, TC *C,
                      uint32_t WIDTH) {

        // Vector is divided among tiles in the tile group
        // As the tile group is two diemsnional, each tile performs
        // (WIDTH / (bsg_tiles_X * bsg_tiles_Y)) multiplications
...
/*
 * This is the 1 dimensional grid of 2 dimensional tile groups version of vector multiplication
 * that multiplies two vectors A and B and stores the result in C.
 *
 * The code assumes a 1-dimensional grid of 2-dimensional tile groups are called.
 * Due to the nature of the compuation (1 dimensional vector multiplication), there
 ...
 */
template <typename TA, typename TB, typename TC>
int __attribute__ ((noinline)) kernel_vector_mul_1D_grid_2D_tile_groups(TA *A, TB *B, TC *C,
        ...
        // As the tile group is two diemsnional, each tile performs
        // (block_size_x / (bsg_tiles_X * bsg_tiles_Y)) multiplications
